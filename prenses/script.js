...
window.toggleFavorite = async function (event, id) {
    ...
    if (note) {
        try {
            await setDoc(doc(db, "notes_kizim", id), { ...note, isFavorite: !note.isFavorite });
        } catch (e) {
...
window.saveNote = async function () {
    ...
    try {
        ...
        await setDoc(doc(db, "notes_kizim", noteData.id), noteData);
...
window.deleteNote = async function (id) {
    if (confirm('Bu notu silmek istediğinize emin misiniz?')) {
        try {
            await deleteDoc(doc(db, "notes_kizim", id));
        } catch (e) {
            console.error(e);
...
window.deleteGroup = async function (groupName, e) {
    ...
    if (confirm(`"${groupName}" grubunu silmek istediğinize emin misiniz?`)) {
        ...
        try {
            await setDoc(doc(db, "metadata_kizim", "groups"), { list: newGroups });
        } catch (e) {
            console.error(e);
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
        const q = query(collection(db, "notes_kizim"));
                ...
                try {
                    ...
                    if (backup.notes && Array.isArray(backup.notes)) {
                        ...
                        for (const n of backup.notes) {
                            await setDoc(doc(db, "notes_kizim", n.id || Date.now().toString()), n);
                            count++;
                        }
                    ...
                    if (backup.groups && Array.isArray(backup.groups)) {
                        await setDoc(doc(db, "metadata_kizim", "groups"), { list: backup.groups });
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        alert("Not Defterim (Kızım) v24 - İyi Çalışmalar Prenses! 🎀");
    });

    // --- Data Listeners ---
    // --- Duplicate Data Listener Removed ---

    onSnapshot(doc(db, "metadata_kizim", "groups"), (doc) => {
        if (doc.exists()) {
